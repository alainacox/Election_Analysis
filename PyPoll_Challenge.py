...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
            cy_votes = county_votes[county_name]
            #print(f'County votes={cy_votes}')
            cy_percentage = float(cy_votes) / float(total_votes) * 100 
            #print(f'County votes={cy_votes} percentage={cy_percentage:.1f}')
            if (cy_votes > winning_county_vote) and (cy_percentage > county_percentage):
                winning_county_vote = cy_votes
                county_percentage = cy_percentage
                winning_county = county_name
            county_results=(f"{county_name}: {cy_percentage:.1f}% ({cy_votes:,})\n")
    ...
    winning_county_summary = (
        ...
        f"Winning Vote Count: {winning_county_vote:,}\n"
